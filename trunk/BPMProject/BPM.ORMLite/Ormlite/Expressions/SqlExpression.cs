...
namespace ServiceStack.OrmLite
{
    public abstract partial class SqlExpression<T> : ISqlExpression
    {
        ...
        public virtual SqlExpression<T> GroupBy(string groupBy, bool SqlVerifyFragment=true)
        {
            if (SqlVerifyFragment)
            {
        ...
        /// objectWithProperties
        /// </typeparam>
        public virtual SqlExpression<T> AddUpdate<TKey>(Expression<Func<T, TKey>> fields)
        {
            ...
            {
                var addFields = Visit(fields).ToString().Split(',').ToList();
        ...
        public string FromExpression
        {
            get
            {
                ...
                return string.IsNullOrEmpty(fromExpression)
                    ? " \nFROM " + DialectProvider.GetQuotedTableName(modelDef) + " WITH(NOLOCK) "
                    : fromExpression;
            }
        ...
        public string TableName
        {
            get
            {
                return string.IsNullOrEmpty(tableName) ? DialectProvider.GetQuotedTableName(modelDef) : tableName;
            }
            set
            {
                tableName = value;
        ...
        /// <summary>
        /// 设置需要更新的字段（只有设置的字段才会更新）
        /// </summary>
        /// <param name="memberExpressions">被更新的字段</param>
        public void AddUpdateField(params Expression<Func<T, dynamic>>[] memberExpressions)
        {
            foreach (var exp in memberExpressions)
            {
                // 获取属性访问表达式
                ...
                if (exp != null)
                {
                    ...
                    {
                        UnaryExpression unaryExpr = exp.Body as UnaryExpression;
                        if (unaryExpr != null)
                        {
                ...
                // 取出Db字段Name，并增加到UpdateFields中
                if (memberExpr != null)
                {
                    // 获取属性名
                    string propertyName = memberExpr.Member.Name;
                    // 根据属性，查找对应的Db的字段Name
                    string newFieldName = modelDef.FieldDefinitions.First(f => f.Name == propertyName).FieldName;
                    if (!UpdateFields.Contains(newFieldName))
                    {
                        UpdateFields.Add(newFieldName);
                    }
                }
            }
        }
        ...
        private string ConvertToEnum(Type enumType, string enumStr, object otherExpr)
        {
            ...
            // modified by liujinfu
            long numvericVal;
            var result = Int64.TryParse(enumStr, out numvericVal)
                ? numvericVal.ToString()
                : OrmLiteConfig.DialectProvider.GetQuotedValue(otherExpr, otherExpr.GetType());
            return result;
        }
        ...
        private bool IsColumnAccess(MethodCallExpression m)
        {
            ...
            //修改类型判断由单一的T,变为tableDefs。 by chenhaitao 2014-09-12
            return exp != null
                && exp.Expression != null
                && tableDefs.Any(td => td.ModelType == exp.Expression.Type)
        ...
        private void BuildSelectExpression(string fields, bool distinct)
        {
            if (!string.IsNullOrEmpty(fields))
                ...
                SelectFields = OrmLiteConfig.DialectProvider.GetColumnNames(modelDef);
        ...
        protected virtual object VisitSqlMethodCall(MethodCallExpression m)
        {
            ...
            switch (m.Method.Name)
            {
                ...
                case "Avg":
                    ...
                    break;
                case "Cast":
                    statement = string.Format("{0}({1} AS {2})",
                                         m.Method.Name,
                                         quotedColName,
                                         args.Count == 1 ? string.Format("{0}", args[0]) : "");
                    break;
