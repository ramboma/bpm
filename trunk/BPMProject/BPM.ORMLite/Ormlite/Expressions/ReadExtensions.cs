...
namespace ServiceStack.OrmLite
{
    public static class ReadExtensions
    {
        ...
        internal static List<Into> Select<Into, From>(this IDbCommand dbCmd, SqlExpression<From> expression, bool customizeSelect = false)
        {
            
            string sql = "";
            //使用自定义的SelectField作为映射  by chenhaitao 20140825
            if (customizeSelect)
            {
                sql = expression.ToSelectStatement();
            }
            else
            {
                sql=expression.SelectInto<Into>();
            }
        ...
        {
            string sql = "";
            if (!string.IsNullOrEmpty(expression.GroupByExpression) && !string.IsNullOrEmpty(expression.GroupByField))
            {
                sql = string.Format("select count(1) from({0}) as cc", "select " + expression.GroupByField + expression.BodyExpression);
            }
            else
            {
                sql = expression.ToCountStatement();
            }
            return dbCmd.Scalar<long>(sql);
        }
            ...
            using (dataReader)
            {
                if (dataReader.Read())
                {
                    ...
                    for (int i = 0; i < dataReader.FieldCount; i++)
                    {
                        ...
                        //add by yanglei sqlserver 关键字 作为列的字段
                        if (fieldDef == null)
                        {
