...
namespace ServiceStack.OrmLite
{
        ...
        where TDialect : IOrmLiteDialectProvider
    {
            ...
            if (dataReader.IsDBNull(colIndex))
            {
                if (fieldDef.IsNullable)
                {
                    if(fieldDef.FieldType==typeof(string))
                        fieldDef.SetValueFn(instance, "");
                    else fieldDef.SetValueFn(instance, null);
                }
                else
                {
        ...
        protected virtual object GetValue<T>(FieldDefinition fieldDef, object obj)
        {
            ...
            if (value != null)
            {
                ...
                if (fieldDef.FieldType.IsEnum)
                {
                    ...
                    return Convert.ToInt32(Enum.Parse(fieldDef.FieldType, value.ToString())).ToString();
        ...
        public virtual string GetQuotedValue(object value, Type fieldType)
        {
            ...
            if (fieldType.IsEnum)
            {
                return Convert.ToInt32(Enum.Parse(fieldType, value.ToString())).ToString();
            }
